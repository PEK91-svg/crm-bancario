...
const formSchema = z.object({
    ...
    circuit: z.enum(["visa", "mastercard", "maestro"]).default("visa"),
...
export default function NewCartaPrepagataPage() {
    const router = useRouter()
    const [loading, setLoading] = useState(false)

    const form = useForm<z.infer<typeof formSchema>>({
        resolver: zodResolver(formSchema),
        defaultValues: { circuit: "visa", status: "pending_activation" },
    })

    async function onSubmit(values: z.infer<typeof formSchema>) {
        setLoading(true)
        try {
            await api.post("cards", { json: { ...values, type: "prepagata" } }).json()
            toast.success("Carta prepagata creata con successo")
            router.push("/prodotti/carte-prepagate")
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-10 max-w-2xl">
            <div className="mb-8">
                <h1 className="text-3xl font-bold tracking-tight">Nuova Carta Prepagata</h1>
                <p className="text-muted-foreground">Inserisci i dati della nuova carta prepagata.</p>
            </div>
            <div className="rounded-xl border bg-card text-card-foreground shadow p-6">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="lastFourDigits" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="9012" maxLength={4} {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="availableCredit" render={({ field }) => (
                                <FormItem>
                            ...
                            <FormField control={form.control} name="monthlyLimit" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="2000.00" {...field} /></FormControl>
                                </FormItem>
                            )} />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="productName" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="Prepagata Young" {...field} /></FormControl>
                                </FormItem>
                            )} />
