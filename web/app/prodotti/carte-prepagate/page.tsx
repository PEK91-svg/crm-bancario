...
export default function CartePrepagatePage() {
    const [search, setSearch] = useState("")

    const { data: response, isLoading } = useQuery({
        queryKey: ['cards-prepagata', search],
        queryFn: () => api.get('cards', { searchParams: { type: 'prepagata', ...(search ? { search } : {}) } }).json<{ data: any[] }>()
    })
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-pink-400 to-rose-500 bg-clip-text text-transparent">Carte Prepagate</h1>
                    <p className="text-muted-foreground mt-1">Gestione carte prepagate e ricaricabili</p>
                </div>
                <Link href="/prodotti/carte-prepagate/new">
                    <Button className="gap-2 bg-pink-600 hover:bg-pink-500 text-white border-0 shadow-lg shadow-pink-500/25">
            ...
            <div className="flex items-center gap-4 bg-card/30 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
                <div className="relative flex-1 max-w-sm">
                    ...
                    <Input
                        ...
                        className="pl-9 bg-black/20 border-white/10 focus:border-pink-500/50"
                        value={search}
                ...
                <Table>
                    <TableHeader className="bg-white/5">
                        <TableRow className="hover:bg-transparent border-white/5">
                            ...
                            <TableHead>Saldo Disponibile</TableHead>
                        ...
                        {isLoading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={5} className="text-center py-12 text-muted-foreground">
                                    Nessuna carta prepagata trovata
                                </TableCell>
                            </TableRow>
                        ) : (
                            cards.map((card: any) => (
                                <TableRow key={card.id} className="border-white/5 hover:bg-white/5 transition-colors">
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            <div className="h-9 w-9 rounded-full bg-gradient-to-br from-pink-700 to-rose-800 flex items-center justify-center border border-white/10">
                                                <Wallet className="h-4 w-4 text-pink-200" />
                                            </div>
                                            <div>
