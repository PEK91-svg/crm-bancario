...
const formSchema = z.object({
    ...
    circuit: z.enum(["visa", "mastercard", "american_express"]).default("visa"),
...
export default function NewCartaCreditoPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        setLoading(true)
        try {
            await api.post("cards", { json: { ...values, type: "credito" } }).json()
            toast.success("Carta di credito creata con successo")
            router.push("/prodotti/carte-credito")
        } catch (error) {
    ...
    return (
        <div className="container mx-auto py-10 max-w-2xl">
            <div className="mb-8">
                <h1 className="text-3xl font-bold tracking-tight">Nuova Carta di Credito</h1>
                <p className="text-muted-foreground">Inserisci i dati della nuova carta di credito.</p>
            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <FormField control={form.control} name="monthlyLimit" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="3000.00" {...field} /></FormControl>
                                </FormItem>
                            )} />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="productName" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input placeholder="Carta Oro" {...field} /></FormControl>
                                </FormItem>
                            )} />
